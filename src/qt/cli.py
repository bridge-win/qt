...
strategy_app = typer.Typer(help="Solution-gallery strategies (A=DCA, B=Capitulation, C=Trend, D=Carry)")
...
def strategy_run_cmd(
    ...
    """Backtest one of the four solution-gallery strategies on local data."""

    from qt.data.store import ParquetStore
    from qt.strategies import BasisCarry, Capitulation, SmartDCA, WeeklyTrend
    ...
    if which in {"a", "dca", "smart_dca"}:
        from qt.strategies import SmartDCAConfig
        out = SmartDCA(SmartDCAConfig(initial_cash=initial_cash)).run(
            ohlcv,
            fear_greed=_read("sentiment", "fear_greed", "fear_greed"),
            mvrv_z=_read("onchain", "glassnode_mvrv_z", "mvrv_z"),
        ...
        from qt.strategies import CapitulationConfig
        out = Capitulation(CapitulationConfig(initial_cash=initial_cash)).run(
            ohlcv,
            fear_greed=_read("sentiment", "fear_greed", "fear_greed"),
            mvrv_z=_read("onchain", "glassnode_mvrv_z", "mvrv_z"),
            sopr=_read("onchain", "glassnode_sopr_adj", "sopr_adj"),
        ...
        from qt.strategies import WeeklyTrendConfig
        out = WeeklyTrend(WeeklyTrendConfig(initial_cash=initial_cash)).run(ohlcv)
    elif which in {"d", "carry", "basis", "basis_carry"}:
        from qt.strategies import BasisCarryConfig
        funding = _read("derivatives", "binance_funding", "funding_rate")
        if funding is None:
            console.print("[red]basis carry requires funding-rate history[/]")
            raise typer.Exit(2)
        out = BasisCarry(BasisCarryConfig(initial_cash=initial_cash)).run(
