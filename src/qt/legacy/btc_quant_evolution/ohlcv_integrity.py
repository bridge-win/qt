...
def filter_ohlcv_rows(rows: list[dict[str, Any]], timerange: str) -> list[dict[str, Any]]:
    match = re.fullmatch(r"(\d{8})-(\d{8})", timerange)
    if match is None:
    ...
    return [row for row in rows if start <= _normalize_datetime(row["date"]) < end]
...
def main() -> int:
    ...
    if args.timerange is not None:
        rows = filter_ohlcv_rows(rows, args.timerange)
