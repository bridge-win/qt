...
from qt.legacy.btc_quant_main.ohlcv_integrity import data_file_path, timeframe_to_seconds
from qt.legacy.btc_quant_main.readiness import write_readiness_report
from qt.legacy.btc_quant_main.regime import write_regime_report
from qt.legacy.btc_quant_main.reports import write_backtest_summary
from qt.legacy.btc_quant_main.robustness import RobustnessPolicy, write_robustness_report
...
def build_research_plan(request: BacktestRequest, *, as_of_date: date | None = None) -> list[PipelineStep]:
    ...
    steps = [
        ...
        PipelineStep(name="backtest", command=_backtest_command(request, analysis_timerange=analysis_timerange)),
    ]
...
def build_artifact_manifest(*, root_dir: Path, request: BacktestRequest) -> dict[str, dict[str, Any]]:
    artifact_paths = {
        ...
        "strategy": root_dir / "user_data" / "strategies" / f"{request.strategy}.py",
    }
...
def _backtest_command(request: BacktestRequest, *, analysis_timerange: str) -> list[str]:
    return [
        *_freqtrade_command("backtesting", env_overrides=_profile_env(request)),
        "--config",
        CONFIG_PATH,
        "--strategy",
        request.strategy,
...
def _ohlcv_integrity_command(request: BacktestRequest) -> list[str]:
    return [
        ".venv/bin/python",
        "-m",
...
def _recursive_command(request: BacktestRequest, *, analysis_timerange: str) -> list[str]:
    return [
        *_freqtrade_command("recursive-analysis", env_overrides=_profile_env(request)),
        "--config",
        CONFIG_PATH,
        "--strategy",
        request.strategy,
...
def _profile_env(request: BacktestRequest) -> list[str]:
    if request.profile is None:
        return []
    return [f"FREQTRADE_PROFILE={request.profile}"]
...
def _runtime_profile_artifact(payload: dict[str, Any], profile_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "definition_sha256": hashlib.sha256(
            json.dumps(asdict(source_profile), sort_keys=True, separators=(",", ":")).encode("utf-8")
        ).hexdigest(),
    }
