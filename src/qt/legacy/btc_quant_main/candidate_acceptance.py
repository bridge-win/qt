...
def write_candidate_acceptance_report(
    *,
    ...
    parameter_stability = _parameter_stability(splits if isinstance(splits, list) else [], root_dir=root)
...
def _return_confidence_check(
    ...
    *,
    ...
    if isinstance(p05, (int, float)) and p05 < policy.min_bootstrap_mean_p05_pct:
    ...
    if (
        isinstance(loss_probability, (int, float))
        and loss_probability > policy.max_bootstrap_loss_probability
    ):
        failed_reasons.append(
            f"bootstrap_loss_probability above {policy.max_bootstrap_loss_probability}"
        )
...
def _return_confidence(
    ...
    *,
    ...
    returns = _split_profit_values(splits)
    if not returns:
        return {
...
def _split_profit_values(splits: list[object]) -> list[float]:
    values = []
    for split in splits:
        ...
        if isinstance(profit_total_pct, (int, float)):
            values.append(float(profit_total_pct))
    return values
