...
def evaluate_strategy_source_risk(
    ...
    strategy_risk = {
        ...
        "max_allowed_drawdown": _number_or_none(max_drawdown.get("max_allowed_drawdown")),
        "max_drawdown_protection": bool(max_drawdown),
        "risk_per_trade_default": _number_or_none(_source_parameter_default(risk_per_trade)),
        "risk_per_trade_max": _number_or_none(_source_parameter_high(risk_per_trade)),
...
def _source_parameter_default(value: object) -> object:
    if isinstance(value, ast.Call):
        for keyword in value.keywords:
            if keyword.arg == "default":
                return _literal_or_node(keyword.value)
    return _parameter_default(value)


def _source_parameter_high(value: object) -> object:
    if isinstance(value, ast.Call):
        if len(value.args) >= 2:
            return _literal_or_node(value.args[1])
        for keyword in value.keywords:
            if keyword.arg == "high":
                return _literal_or_node(keyword.value)
