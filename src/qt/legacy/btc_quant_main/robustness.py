...
def evaluate_trade_robustness(
    *,
    ...
    chronological_max_drawdown_pct = _max_drawdown_pct(returns) if returns else None
    loss_cluster_returns = sorted(returns)
    loss_cluster_max_drawdown_pct = _max_drawdown_pct(loss_cluster_returns) if returns else None
    compounded_trade_profit_pct = _compounded_trade_profit_pct(returns) if returns else None
    loss_cluster_compounded_trade_profit_pct = (
        _compounded_trade_profit_pct(loss_cluster_returns) if returns else None
    )
    cost_stress_compounded_trade_profit_pct = (
        _compounded_trade_profit_pct(cost_stressed_returns) if returns else None
    )
    ...
    result["robustness_gate"] = _evaluate_gate(result, policy)
    return result
...
def _evaluate_gate(result: dict[str, Any], policy: RobustnessPolicy) -> dict[str, Any]:
    failed_reasons = []
    ...
    if (
        isinstance(loss_cluster_max_drawdown_pct, (int, float))
    ...
    if (
        isinstance(cost_stress_compounded_trade_profit_pct, (int, float))
